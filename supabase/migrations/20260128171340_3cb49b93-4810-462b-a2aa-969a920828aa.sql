-- Update reports_aggregate to support both 'remove' and 'sale' action types
-- Also add unit counts for timeline chart
...
BEGIN
  ...
  -- Count ALL remove/sale actions for quantity, not just those with financial data
  ...
  -- Support both 'remove' and 'sale' action types
  ...
  BEGIN
    SELECT jsonb_agg(jsonb_build_object(
        'supplier_id', s.id,
        'supplier_name', COALESCE(s.name, 'ללא ספק'),
